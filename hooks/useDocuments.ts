...
export interface PreviewPageUrl {
  url: string;
  expiresIn: number;
}

// Fetch preview info for a document
export function useDocumentPreviewInfo(documentId: string) {
  const { getToken } = useAuth();
  const apiClient = createClientApiClient(getToken);

  return useQuery({
    queryKey: [...documentKeys.detail(documentId), "preview-info"] as const,
    queryFn: async () => {
      const response = await apiClient.get<PreviewInfo>(
        `/documents/${documentId}/preview/info`
      );
      return response.data;
    },
    enabled: !!documentId,
...
// Fetch signed URL for a specific preview page
export function useDocumentPreviewPage(documentId: string, pageNumber: number) {
  const { getToken } = useAuth();
  const apiClient = createClientApiClient(getToken);
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<PreviewPageUrl>(
        `/documents/${documentId}/preview/${pageNumber}`
      );
      return response.data;
    },
    enabled: !!documentId && pageNumber > 0,
    // Short stale time since URLs expire after 15 minutes
    staleTime: 10 * 60 * 1000, // 10 minutes
    // Refetch before URL expires
    refetchInterval: 12 * 60 * 1000, // 12 minutes
  });
}
  ...
  return useMutation({
    mutationFn: async (documentId: string) => {
      const response = await apiClient.post<{
        pageCount: number;
        message: string;
      }>(`/documents/${documentId}/generate-previews`);
      return response.data;
    },
    onSuccess: (_, documentId) => {
      // Invalidate preview info to refresh page count
      queryClient.invalidateQueries({
        queryKey: [...documentKeys.detail(documentId), "preview-info"],
      });
    },
  });
}
